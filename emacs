...
(defun load-ipynb-readonly ()
  "Activate read only mode if the buffer name ends in .ipynb."
  (let ((buf-name (buffer-name (current-buffer))))
    (when (string-suffix-p "ipynb" buf-name)
      (read-only-mode 1))))
(add-hook 'find-file-hook 'load-ipynb-readonly)
...
(when (require 'markdown-mode nil t)
  (when (boundp 'markdown-mode-map)
    ...
    (define-key markdown-mode-map "\M-\C-n" 'markdown-move-list-item-down)
    (define-key markdown-mode-map "\M-\C-p" 'markdown-move-list-item-up)
    (define-key markdown-mode-map [C-M-left] 'markdown-promote-list-item)
    (define-key markdown-mode-map [C-M-right] 'markdown-demote-list-item)
    (define-key markdown-mode-map "\C-c\C-n" 'markdown-insert-list-item)
    (define-key markdown-mode-map "\C-o" 'markdown-insert-list-item)
